...
if($crids = $arc->arc_crids()){//需要对当前用户扣值
	...
	if(!empty($commu['setting']['autoarc'])){//不自动扣值的情况：如有前导页，进前导页，否则提示出订阅链接，选择是否订阅
		if(!$pretpl) message('购买此文档需要支付积分： '.$cridstr."<br><br><a href=\"subscribe.php?aid=$aid\">>>".lang('subscribe')."</a>");
		$ispre = 1;
...
if(!$enablestatic && $cache1circle){
	$cachefile = htmlcac_dir($ispre ? 'pre' : 'arc',date('Ym',$arc->archive['createdate']),1).cac_namepre($arc->aid).'_'.$page.'.php';//前导页与正式页,附加页的缓存需要区分开????????????????????????

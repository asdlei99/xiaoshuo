...
if(empty($submitmode)){
	...
	'aread' => lang('read_state'),
	'areply' => lang('reply_state'),
	);
	foreach($ucotypes as $k => $v) if($v['cclass'] == 'reply') $filtersarr['uccid'.$k] = $v['cname'];
	...
	$imuidsarr = array();
	foreach($inmurls as $k => $v) if(in_array($v['uclass'],array('offerdetail','custom'))) $imuidsarr[$k] = '<b>'.$v['cname'].'</b>&nbsp; '.$v['remark'];
	...
	foreach(array('cuids','chids','filters','lists','operates','imuids',) as $var){
		$murlnew['setting'][$var] = empty(${$var.'new'}) ? '' : implode(',',${$var.'new'});
	}
	$murlnew['url'] = "?action=areplys&nmuid=$muid";
}
?>
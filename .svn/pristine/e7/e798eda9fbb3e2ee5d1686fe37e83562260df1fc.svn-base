		...
		if(!submitcheck('barcsedit')){
			if(empty($u_tplname)){
				...
				if(empty($u_operates) || in_array('catalog',$u_operates)){
					trbasic("<input class=\"checkbox\" type=\"checkbox\" name=\"arcdeal[caid]\" value=\"1\">&nbsp;".lang('catalog'),'arccaid',umakeoption(ucaidsarr()),'select');
				}
				foreach($cotypes as $k => $v){
					if(empty($v['self_reg'])){
						if(empty($u_operates) || in_array('ccid'.$k,$u_operates)){
							$ccidsarr = array('0' => array('title' => lang('noset'))) + uccidsarr($k);
							trbasic("<input class=\"checkbox\" type=\"checkbox\" name=\"arcdeal[ccid$k]\" value=\"1\">&nbsp;$v[cname]",'arcccid'.$v['coid'],umakeoption($ccidsarr),'select');
						}
					}
				}

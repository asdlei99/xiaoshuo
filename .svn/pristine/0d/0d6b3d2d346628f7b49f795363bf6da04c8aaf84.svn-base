...
if(empty($u_tplname) || !empty($u_onlyview)){
	...
	if($caids) $wheresql .= " AND caid ".multi_str($caids);
	...
	foreach($cotypes as $coid => $cotype){
		if(!empty(${'u_ccids'.$coid})){
			...
			if(!empty($cotype['self_reg'])){
				if($tempstr = self_sqlstr($coid,$ccids,'')) $wheresql .= " AND $tempstr";
				else $no_list = true;
			}else $wheresql .= " AND ccid$coid ".multi_str($ccids);
		}
	}
	$wheresql = "WHERE ".(empty($no_list) ? $wheresql : '1=0');
	
	if(empty($u_tplname)){
		echo mform_str($action.'archivesedit',"?action=albums&nmuid=$nmuid&page=$page");
		mtabheader_e();
		...
		mtabheader(empty($u_mtitle) ? lang('publicalbumlist') : $u_mtitle,'','',30);
		...
		while($row = $db->fetch_array($query)){
			...
			$adminstr = "<a href=\"addpre.php?aid=$row[aid]\" onclick=\"return floatwin('open_inarchive',this)\">".lang('add')."</a>";

...
if(!$mid) message('choosereplyofmember');
$cuid = $db->result_one("SELECT c.reply FROM {$tblprefix}members m LEFT JOIN {$tblprefix}mchannels c ON c.mchid=m.mchid WHERE m.mid='$mid'");
if(!$cuid || !($mcommu = read_cache('mcommu',$cuid))) message('setmemcommitem');
if(empty($mcommu['ucadd'])){
	$actuser = new cls_userinfo;
	$actuser->activeuser($mid);
	if(!$actuser->info['mid']) message('choosereplyofmember');
	if(!$curuser->pmbypmids('cuadd',$mcommu['setting']['apmid'])) message('younoreplypermis');
	$fieldsarr = empty($mcommu['setting']['fields']) ? array() : explode(',',$mcommu['setting']['fields']);
	
	if(!submitcheck('newcommu')){
		//对重复回复及频繁回复的处理
		if(!empty($mcommu['setting']['norepeat']) && $cid = $db->result_one("SELECT cid FROM {$tblprefix}mreplys WHERE mid='$mid' AND fromid='$memberid' ORDER BY cid")){
			 message('dorepeataddreply');
		}
		if(!empty($mcommu['setting']['repeattime']) && $cid = $db->result_one("SELECT cid FROM {$tblprefix}mreplys WHERE mid='$mid' AND fromid='$memberid' AND createdate>'".($timestamp - 60 * $mcommu['setting']['repeattime'])."' ORDER BY cid DESC")){
			message('addreplyoverquick');
		}
		if(!($tplname = @$mcommu['addtpl'])){
			load_cache('mlangs,mrfields');
			include_once M_ROOT."./include/fields.fun.php";
			include_once M_ROOT."./include/fields.cls.php";
			include_once M_ROOT."./include/cheader.inc.php";
			_header();
			
			if(!$oldmsg = $db->fetch_one("SELECT * FROM {$tblprefix}mreplys WHERE fromid='$memberid' AND cuid='$mcommu[cuid]' ORDER BY cid DESC LIMIT 0,1")) $oldmsg = array();
			mtabheader(lang('add').$mcommu['cname'],'replyadd',"mreply.php?mid=$mid$forwardstr",2,1,1);
			$submitstr = '';
			$a_field = new cls_field;
			foreach($mrfields as $k => $v){
		...
		//分析是否允许重复添加
		if(!empty($mcommu['setting']['norepeat']) && $cid = $db->result_one("SELECT cid FROM {$tblprefix}mreplys WHERE mid='$mid' AND fromid='$memberid' ORDER BY cid")){
			mcmessage('dorepeataddreply',axaction(2,M_REFERER));
		}
		if(!empty($mcommu['setting']['repeattime']) && $cid = $db->result_one("SELECT cid FROM {$tblprefix}mreplys WHERE mid='$mid' AND fromid='$memberid' AND createdate>'".($timestamp - 60 * $mcommu['setting']['repeattime'])."' ORDER BY cid DESC")){
			mcmessage('addreplyoverquick',axaction(2,M_REFERER));
		}

...
if(empty($commu['umdetail'])){
	...
	if(!submitcheck('banswerdetail')){
		mtabheader(lang('editanswer'),'answerdetail',"?action=answer&cid=$cid$forwardstr");
		mtrbasic(lang('questiontitle'),'',$aedit->archive['subject'],'');
		mtrbasic(lang('answercontent'),'answernew[answer]',br2nl($answer['answer']),'btextarea');
		mtabfooter($amode ? '' : 'banswerdetail');
	}else{

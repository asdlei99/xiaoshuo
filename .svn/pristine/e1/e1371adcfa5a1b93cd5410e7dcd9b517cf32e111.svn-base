...
if(!submitcheck('buserfilesedit')){
	...
	echo mform_str($action.'arcsedit',"adminm.php?action=userfiles");
	mtabheader_e();
	...
	$multi = multi($itemcount, $mrowpp, $page, "adminm.php?action=userfiles$filterstr");

	mtabheader(lang('attachmentlist'),'','',9);
	mtrcategory(array("<input class=\"checkbox\" type=\"checkbox\" name=\"chkall\" class=\"category\" onclick=\"checkall(this.form, 'selectid', 'chkall')\">".lang('del'),array(lang('cname'),'left'),lang('type'),lang('size_k'),lang('preview'),lang('thumb'),lang('uploaddate'),lang('source')));
	echo $itemstr;
	mtabfooter();
	echo $multi;
	echo "<br><input class=\"button\" type=\"submit\" name=\"buserfilesedit\" value=\"".lang('submit')."\"></form>";
}else{
	empty($selectid) && mcmessage('selectarchive',"adminm.php?action=userfiles&page=$page$filterstr");
	$items = array();
	...
	if(!empty($select_all)){
		...
		if($npage <= $pages){
			...
			mcmessage('operating'."<br>
					...
					<a href=\"adminm.php?action=userfiles&page=$page$filterstr\">>>".lang('pause')."</a>",
					"adminm.php?action=userfiles&page=$page$filterstr$transtr",
					500);
		}
	}
	mcmessage('archiveoperatefinish',"adminm.php?action=userfiles&page=$page$filterstr",500);

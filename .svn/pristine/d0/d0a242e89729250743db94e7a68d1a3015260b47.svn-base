...
if(empty($deal)){
	...
	if(!submitcheck('bflinksedit')){
		...
		echo mform_str($action.'arcsedit',"?action=mflinks&page=$page");
		mtabheader_e();
		...
		while($item = $db->fetch_array($query)){
			...
			$itemstr .= "<tr><td class=\"item\" width=\"30\"><input class=\"checkbox\" type=\"checkbox\" name=\"selectid[$item[cid]]\" value=\"$item[cid]\"></td>\n".
				...
				"<td class=\"item2\"><a href=\"mspace/index.php?mid=$item[mid]\" target=\"_blank\">$item[mname]</a></td>\n".
		...
		mtrcategory(array("<input class=\"checkbox\" type=\"checkbox\" name=\"chkall\" class=\"category\" onclick=\"checkall(this.form, 'selectid', 'chkall')\">",lang('id'),array(lang('member'),'left'),lang('uclass'),lang('check'),lang('addtime'),lang('operate')));
		echo $itemstr;
		mtabfooter();
		echo $multi;
		mtabheader(lang('operateitem'));
		mtrbasic(lang('choose_item'),'',"<input class=\"checkbox\" type=\"checkbox\" name=\"flinkdeal[delete]\" value=\"1\">&nbsp;".lang('delete').$listname,'');
		mtrbasic("<input class=\"checkbox\" type=\"checkbox\" name=\"flinkdeal[ucid]\" value=\"1\">&nbsp;".lang('uclass'),'flinkucid',makeoption(array('0' => lang('cancelcoclass')) + $ucidsarr),'select');
		mtabfooter('bflinksedit');
	}else{

...
if(empty($commu['uconfig'])){
	if($action == 'commudetail'){
		...
		trbasic(lang('add_comment'),'','{cms_abs}report.php?aid={aid}','');
		trbasic(lang('admin_arc_comment'),'','{cms_abs}report.php?action=admin&aid={aid}','');
		trbasic(lang('edit_point_comment'),'','{cms_abs}report.php?action=edit&aid={aid}&cid={cid}','');
	}

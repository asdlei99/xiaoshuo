...
'next','chid','caid','mid','aid','func','mpfunc','sqlstr','vid','vsource','vids','chdata','js','checked','nsid','cnid','cnsource','level','caids',
'validperiod','thumb','nocp','asc','chids','nochids','val','tname','disabled',);//只要为空或为0就可以清除的参数
...
if($action == 'mtagadd'){
	...
	if(!submitcheck('bmtagsedit')){
		...
		$helpstr = $ttype == 'rtag' ? '' : "&nbsp; &nbsp; [<a href=\"./taghelp.html".($tclass ? '#'.str_replace('tag','',$ttype).'_'.$tclass : '')."\" target=\"08cmstaghelp\">".lang('help')."</a>]";

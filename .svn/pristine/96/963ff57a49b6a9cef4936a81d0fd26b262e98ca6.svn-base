...
if(!$fsave){
	...
	if(!$isfunc){
		...
		$submitstr .= makesubmitstr('fieldnew[vdefault]',0,'int',0,11);
		...
		$submitstr .= makesubmitstr('fieldnew[min]',0,'int',0,11);
		$submitstr .= makesubmitstr('fieldnew[max]',0,'int',0,11);
	...
	if($fnew){
		...
		$db->query("ALTER TABLE $fconfigarr[altertable] ADD $fieldnew[ename] int(11) NOT NULL default '0'");
	}else{
	...
	if(!$isfunc){
		...
		foreach(array('vdefault','min','max') as $key){
			...
			if($fieldnew[$key] != '') $fieldnew[$key] = intval($fieldnew[$key]);
		}

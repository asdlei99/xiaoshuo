	...
	if(!submitcheck('bfriendsedit')){
		...
		while($item = $db->fetch_array($query)){
			...
			$itemstr .= "<tr><td class=\"item\" width=\"30\"><input class=\"checkbox\" type=\"checkbox\" name=\"selectid[$item[cid]]\" value=\"$item[cid]\"></td>\n".
				...
				"<td class=\"item\"><a href=\"mspace/index.php?mid=$item[fromid]\" target=\"_blank\">$item[fromname]</a></td>\n".
		...
		mtabheader(lang('needlist'),'','',11);
		mtrcategory(array("<input class=\"checkbox\" type=\"checkbox\" name=\"chkall\" class=\"category\" onclick=\"checkall(this.form, 'selectid', 'chkall')\">",lang('id'),lang('membercname'),lang('needtime'),lang('operate')));
		echo $itemstr;
		mtabfooter();
		echo $multi;
		mtabheader(lang('operateitem'));
		mtrbasic(lang('choose_item'),'',"<input class=\"checkbox\" type=\"checkbox\" name=\"frienddeal[delete]\" value=\"1\">&nbsp;".lang('delete').$listname."&nbsp;<input class=\"checkbox\" type=\"checkbox\" name=\"frienddeal[pass]\" value=\"1\">&nbsp;".lang('check').$listname,'');

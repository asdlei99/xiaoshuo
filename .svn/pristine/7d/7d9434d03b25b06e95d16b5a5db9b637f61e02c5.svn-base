...
if(empty($mcommu['umdetail'])){
	...
	if(!submitcheck('newcommu')){
		mtabheader($mcommu['cname'].'&nbsp; &nbsp; '."<a href=\"mspace/index.php?mid=".$oldrow[$amode ? 'fromid' : 'mid']."\" target=\"_blank\">>>&nbsp; ".$oldrow[$amode ? 'fromname' : 'mname']."</a>",'commudetail',"?action=mreply&amode=$amode&cid=$cid",2,1,1);
		$submitstr = '';
		...
		foreach($fields as $k => $v){
			if(empty($u_lists) || in_array($k,$u_lists)){
				if(!$v['isfunc']){
					...
					$a_field->trfield('communew',$noedit);

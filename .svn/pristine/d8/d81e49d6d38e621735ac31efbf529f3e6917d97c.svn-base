...
if($action == 'permissionsedit'){
	if(!submitcheck('bpermissionsedit') && !submitcheck('bpermissionsadd')) {
		...
		trcategory(array(lang('delete'),lang('projectname'),lang('order'),lang('archivebrowse'),lang('cnbrowse'),lang('arc_issue'),lang('farcissue'),lang('cuissue'),lang('atmdown'),lang('mcmenu'),lang('fieldpm'),lang('edit')));
		foreach($permissions as $k => $v){
			echo "<tr class=\"txt\">".
				...
				"<td class=\"txtC w60\"><input class=\"checkbox\" type=\"checkbox\" name=\"permissionsnew[$k][down]\" value=\"1\"".($v['down'] ? " checked" : "")."></td>\n".
				"<td class=\"txtC w60\"><input class=\"checkbox\" type=\"checkbox\" name=\"permissionsnew[$k][menu]\" value=\"1\"".($v['menu'] ? " checked" : "")."></td>\n".
				"<td class=\"txtC w60\"><input class=\"checkbox\" type=\"checkbox\" name=\"permissionsnew[$k][field]\" value=\"1\"".($v['field'] ? " checked" : "")."></td>\n".
		...
		foreach($permissionsnew as $k => $v){
			...
			$sqlstr = '';
			foreach(array('aread','cread','aadd','fadd','cuadd','down','menu','field',) as $var) $sqlstr .= "$var='".(empty($v[$var]) ? 0 : 1)."',";

...
if(empty($deal)){
	...
	if(!submitcheck('bmembersedit')){
		...
		mtabheader(lang('filtersetting').viewcheck('viewdetail',$viewdetail,'tbodyfilter').'&nbsp; &nbsp; '.mstrbutton('bfilter','filter0'),'membersedit',"?action=$action&page=$page");
		echo "<tbody id=\"tbodyfilter\" style=\"display: ".(empty($viewdetail) ? 'none' : '')."\">";
		mtrbasic(lang('memberid'),'mid',$mid);
		mtrbasic(lang('membercname'),'mname',$mname,'text',lang('agsearchkey'));
		...
		mtrrange(lang('adddate'),array('outdays',empty($outdays) ? '' : $outdays,'','&nbsp; '.lang('daybefore').'&nbsp; -&nbsp; ',5),array('indays',empty($indays) ? '' : $indays,'','&nbsp; '.lang('dayin'),5));
		echo "</tbody>";
		mtabfooter();
		...
		while($item = $db->fetch_array($query)){
			...
			$itemstr .= "<tr><td align=\"center\" class=\"item1\" width=\"30\"><input class=\"checkbox\" type=\"checkbox\" name=\"selectid[$item[cid]]\" value=\"$item[cid]\"></td>\n".
				...
				"<td class=\"item1\"><a href=\"mspace/index.php?mid=$item[mid]\" target=\"_blank\">$item[mname]</a></td>\n".
		...
		mtrcategory(array("<input class=\"checkbox\" type=\"checkbox\" name=\"chkall\" class=\"category\" onclick=\"checkall(this.form, 'selectid', 'chkall')\">",lang('id'),lang('member'),lang('uclass'),lang('addtime'),lang('operate')));
		echo $itemstr;
		mtabfooter();
		echo $multi;
		...
		mtrbasic("<input class=\"checkbox\" type=\"checkbox\" name=\"favoritedeal[ucid]\" value=\"1\">&nbsp;".lang('uclass'),'favoriteucid',makeoption(array('0' => lang('cancelcoclass')) + $ucidsarr),'select');
		mtabfooter('bmembersedit');
	}else{

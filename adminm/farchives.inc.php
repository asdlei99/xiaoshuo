...
if(empty($u_tplname) || !empty($u_onlyview)){
	...
	//À¸Ä¿·¶Î§
	if($caid){
		if(!empty($u_caids) && !in_array($caid,$u_caids)) $no_list = 1;
		else $wheresql .= " AND caid='$caid'";
	}elseif(!empty($u_caids)) $wheresql .= " AND caid ".multi_str($u_caids);
	//ÉóºË×´Ì¬·¶Î§
	if($checked != -1){
	...
	$filterstr = '';
	foreach(array('nmuid','caid','qstate','keyword',) as $k) $$k && $filterstr .= "&$k=".rawurlencode(stripslashes($$k));
	foreach(array('checked','valid',) as $k) $$k != -1 && $filterstr .= "&$k=".$$k;
	if(!submitcheck('barcsedit')){
		if(empty($u_tplname)){
			echo mform_str($action.'arcsedit',"?action=farchives&nmuid=$nmuid&page=$page");
			//ËÑË÷Çø¿é
			mtabheader_e();
			...
			if(empty($u_filters) || in_array('catalog',$u_filters)){
				echo "<select style=\"vertical-align: middle;\" name=\"caid\">".makeoption(array(0 => lang('coclass')) + fcaidsarr(),$caid)."</select>&nbsp; ";
			}
			//ÉóºË×´Ì¬
			...
			//ÁÐ±íÇø	
			mtabheader(empty($u_mtitle) ? lang('contentlist') : $u_mtitle,'','',10);
			...
			while($row = $db->fetch_array($query)){
				...
				$catalogstr = @$fcatalogs[$row['caid']]['title'];
				...
				$adminstr = empty($fcatalogs[$row['caid']]['cumode']) ? '-' : ("<a href=\"?action=fconsult&aid=$row[aid]\" onclick=\"return floatwin('open_inarchive',this)\">".lang('admin')."</a>");

...
if(!submitcheck('bpaymanager')){
	...
	mtabheader(lang('mypaymode'),'paymanager','adminm.php?action=paymanager',2,1,1);
	mtrbasic(lang('paymode'),'',makecheckbox('paymodenew[]',$pmodearr,$payarr),'');
	mtrbasic(lang('ordmode'),'',makeradio('ordermodenew',$omodearr,$curuser->info['ordermode']),'');
	mtrbasic('<input name="spmd[1]" type="checkbox" class="checkbox" value="1"'.($sp1<0?'':' checked="checked"').' />'.lang('shipingfee1'),'shipingfee[1]',$sp1<0?0:$sp1);
	mtrbasic('<input name="spmd[2]" type="checkbox" class="checkbox" value="1"'.($sp2<0?'':' checked="checked"').' />'.lang('shipingfee2'),'shipingfee[2]',$sp2<0?0:$sp2);
	mtrbasic('<input name="spmd[3]" type="checkbox" class="checkbox" value="1"'.($sp3<0?'':' checked="checked"').' />'.lang('shipingfee3'),'shipingfee[3]',$sp3<0?0:$sp3);
	mtrbasic(lang('alipay_account'),'alipay_account',$curuser->info['alipay'],'btext');
	mtrbasic(lang('alipay_partner'),'alipay_partner',$curuser->info['alipid']);
	mtrbasic(lang('alipay_keyt'),'alipay_keyt',$curuser->info['alikeyt'],'btext');
	mtrbasic(lang('tenpay_account'),'tenpay_account',$curuser->info['tenpay'],'btext');
	mtrbasic(lang('tenpay_keyt'),'tenpay_keyt',$curuser->info['tenkeyt'],'btext');
	...
	$db->query("UPDATE {$tblprefix}members_sub SET
				 ...
				 WHERE mid=$memberid
				 ");
	mcmessage('paymodefinish','adminm.php?action=paymanager');
}
?>

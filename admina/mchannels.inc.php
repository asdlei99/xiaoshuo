...
if($action == 'mchannelsedit'){
	...
	if(!submitcheck('bmchanneldetail')){
		...
		$itemsarr = array('caid' => lang('relate').lang('catalog'));
		foreach($cotypes as $k => $v) if(!$v['self_reg']) $itemsarr['ccid'.$k] = lang('relate').$v['cname'];
		trbasic(lang('arelates')."<br><input class=\"checkbox\" type=\"checkbox\" name=\"chkall_1\" onclick=\"checkall(this.form,'acoidsnew','chkall_1')\">".lang('selectall'),'',makecheckbox('acoidsnew[]',$itemsarr,empty($mchannel['acoids']) ? array() : explode(',',$mchannel['acoids']),5),'',lang('agrelates'));
		trbasic(lang('crelates')."<br><input class=\"checkbox\" type=\"checkbox\" name=\"chkall_3\" onclick=\"checkall(this.form,'ccoidsnew','chkall_3')\">".lang('selectall'),'',makecheckbox('ccoidsnew[]',$itemsarr,empty($mchannel['ccoids']) ? array() : explode(',',$mchannel['ccoids']),5),'',lang('agrelates'));
		...
		$mchannelnew['acoids'] = empty($acoidsnew) ? '' : implode(',',$acoidsnew);
		$mchannelnew['ccoids'] = empty($ccoidsnew) ? '' : implode(',',$ccoidsnew);
		...
		$db->query("UPDATE {$tblprefix}mchannels SET 
			...
			acoids='$mchannelnew[acoids]',
			ccoids='$mchannelnew[ccoids]',

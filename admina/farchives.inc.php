...
if($action == 'farchivesedit'){
	...
	if(empty($u_tplname) || !empty($u_onlyview)){
		...
		//À¸Ä¿·¶Î§
		if(!empty($caid)){
			if(!in_array(-1,$a_fcaids) && !in_array($caid,$a_fcaids)) $no_list = 1;
			else $wheresql .= ($wheresql ? ' AND ' : '')."caid='$caid'";
		}elseif(!in_array(-1,$a_fcaids)){
			if(!$a_fcaids) $nolist = 1;
			else $wheresql .= ($wheresql ? ' AND ' : '')."caid ".multi_str($a_fcaids);
		}
		//ÉóºË×´Ì¬·¶Î§
		if($checked != -1){
		...
		$filterstr = '';
		foreach(array('nauid','caid','qstate','keyword',) as $k) $$k && $filterstr .= "&$k=".rawurlencode(stripslashes($$k));
		...
		if(!submitcheck('barcsedit')){
			if(empty($u_tplname)){
				...
				//ÁĞ±íÇø	
				tabheader(@$fcatalogs[$caid]['title'].lang('content'),'','',10);
				...
				while($row = $db->fetch_array($query)){
					...
					$adminstr = empty($fcatalogs[$row['caid']]['cumode']) ? '-' : ("<a href=\"?entry=farchive&action=fconsult&aid=$row[aid]\" onclick=\"return floatwin('open_inarchive',this)\">".lang('admin')."</a>");

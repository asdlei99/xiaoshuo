...
$lan_title = lang($sid ? 'subbackareatitle' : 'mbackareatitle');
$aflag = '';
...
if($aflag == 'off'){
	login_msg(lang('no_admin_backarea'),'','error');
...
$a_funcs = $a_caids = $a_mchids = $a_fcaids = $a_cuids = $a_mcuids = $a_matids = $a_vcaids = $a_ucaids = $a_vfcaids = $a_ufcaids = $a_vmchids = $curuser->info['isfounder'] ? array('-1') : array();;
...
if(!$curuser->info['isfounder']){
	...
	if(($a_amcid = @$ausergroup['amcids'][$sid ? $sid : 'm']) && ($a_amconfig = @$amconfigs[$a_amcid])){
		...
		$a_funcs = explode(',',$a_amconfig['funcs']);
		$a_caids = explode(',',$a_amconfig['caids']);
		$a_mchids = explode(',',$a_amconfig['mchids']);
		$a_fcaids = explode(',',$a_amconfig['fcaids']);
		$a_cuids = explode(',',$a_amconfig['cuids']);
		$a_mcuids = explode(',',$a_amconfig['mcuids']);
		$a_matids = explode(',',$a_amconfig['matids']);
		if($a_amconfig['abcustom'] || !in_array('-1',$a_caids)){
			...
			$a_vcaids = array();
			foreach($a_ucaids as $v) $a_vcaids = array_merge($a_vcaids,!$v ? array($v) : pcaidsarr($v,1));//所有显示栏目的上级栏目需要显示出来
			$a_vcaids = array_unique($a_vcaids);
			if(!in_array('-1',$a_caids)) $a_ucaids = array_intersect($a_caids,$a_ucaids);
		}else $a_ucaids = $a_vcaids = array('-1');
...
if(empty($entry) || isset($isframe)){
	...
	foreach($ncatalogs as $caid => $catalog){
		...
		$editstr = array_intersect(array('-1',$caid),$a_ucaids) ? "<em><a href=\"javascript:\" onclick=\"get_operate($caid)\">$catalog[title]</a></em>" : $catalog['title'];
		if($i<$catalog['level']){
			$i++;
			$catastr.="<ul>\n$space	<li>$editstr";
			$space.='	';
	...
	$f_catastr = '';
	$f_pid = 0;
	//var_dump($fcatalogs);exit();
	foreach($fcatalogs as $k => $v){
		if(!array_intersect(array('-1',$k),$a_vfcaids)) continue;
		$editstr = array_intersect(array('-1',$k),$a_ufcaids) ? "<em><a href=\"javascript:\" onclick=\"get_operate($k,1)\">$v[title]</a></em>" : $v['title'];
		if($f_pid){
	...
	<td colspan="2" height="117">
		<div class="area">
			<div id="header">
				...
				<div class="right">
					<div class="headTxt">
						<h1><font><?=lang('awelcome',$curuser->info['mname'])?><?=$foundernobest?></font>[ <a href="?entry=home&isframe=1"><?=lang('adminindex')?></a> | <a href="<?=$cms_abs?>" target="_blank"><?=lang('websiteindex')?></a> | <a href="adminm.php" target="_blank"><?=lang('membercenter1')?></a> | <a href="mspace/index.php?mid=<?=$memberid?>" target="_blank"><?=lang('space')?></a> | <a href="?entry=logout"><?=lang('logoutadmin')?></a> ]</h1>
...
window.onload = function(){
	...
	window.onresize = function(){
		var h = (document.documentElement ? document.documentElement.clientHeight : document.body.clientHeight) - 126;

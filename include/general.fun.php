...
function m_parseurl($url,$source = array(),$clear = 1){//当解析完整路径的时候，请将$clear保持为0
...
function view_arcurl(&$archive,$addno = 0){//处理栏目及子站绑定域名(后带/) $addno-1为全部页面,不返回//可能在某些环节应用此函数的前后顺序需要改变
	//本函数返回单个页面的url或处理需要解析的多个文档页面url
	...
	if(empty($enablestatic)){
		...
		foreach($addnos as $k){
			...
			}else $archive['arcurl'.$k] = ($nsid ? $subsites[$nsid]['dirname'].'/' : ($cnhtmldir ? $cnhtmldir.'/' : '')).$catalog['dirname'].'/'.$archive['arcurl'.$k];//不绑定域名或非子站内容

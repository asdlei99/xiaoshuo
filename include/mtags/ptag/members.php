...
if(!submitcheck('bmtagadd') && !submitcheck('bmtagsdetail') && !submitcheck('bmtagcode')){
	...
	tabheader(lang('catas_relate_setting')."&nbsp;&nbsp;&nbsp;&nbsp;<input class=\"checkbox\" type=\"checkbox\" name=\"viewdetail\" value=\"1\" onclick=\"alterview('catasfilter')\">".lang('view'));
	echo "<tbody id=\"catasfilter\" style=\"display: none;\">";
	$sourcearr = array('0' => lang('nolimitcatas'),'1' => lang('handpoint'),'2' => lang('activecatas'),);
	$catalogs = &$acatalogs;
	sourcemodule(lang('relate_catalog_limited')."&nbsp;&nbsp;&nbsp;<input class=\"checkbox\" type=\"checkbox\" name=\"mtagnew[setting][caidson]\" value=\"1\"".(empty($mtag['setting']['caidson']) ? "" : " checked").">".lang('include_son'),
	...
	//数组参数的处理
	$idvars = array('chids','caids',);
	foreach($cotypes as $k => $cotype) if(!$cotype['self_reg']) $idvars[] = 'ccids'.$k;
	foreach($idvars as $k){

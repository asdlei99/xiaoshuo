...
if(empty($commu['uconfig'])){
	if($action == 'commudetail'){
		...
		trbasic(lang('add_comment'),'','{cms_abs}comment.php?aid={aid}','');
		trbasic(lang('vote_url'),'','{cms_abs}comment.php?action=vote&cid={cid}&option=xx (xx-'.lang('vote_option').')','');
		trbasic(lang('arc_comment_list'),'','{cms_abs}comments.php?aid={aid}','');
	}

		...
		if(!empty($tag['cosource'.$k])){
			...
			if(!empty($ccids)){
				if(empty($cotype['self_reg'])){
					$sqlwhere .= " AND a.ccid$k ".multi_str($ccids);
				}else{
...
function one_image(&$tag){//带url为数组
	...
	if(empty($ret['url']) && empty($tag['emptyurl'])) return $ret;
	if(!empty($ret['url'])){
		...
		$ret['url'] = $ret['url_s'] = view_atmurl($tag['emptyurl']);
...
function arr_catalogs(&$tag){
	...
	if(empty($sourcestr)){
		...
		if($ids = @explode(',',$tag[$listby ? 'ccids'.$listby : 'caids'])){
		...
		if(!empty($tag['wherestr'])) $sqlwhere .= " AND ".$tag['wherestr'];
	}elseif($sourcestr == 4){
	...
	while($row = $db->fetch_array($query)){
		...
		$cnstr = cnstr($midarr);
		$row = cn_parsearr($cnstr,$nsid,$listby);//??????????????????????????????
...
function one_cnode(&$tag){
	...
	$nsid = empty($tag['nsid']) ? 0 : intval($tag['nsid']);
	if(!$nsid){
		$nsid = isset($_midarr['nsid']) ? $_midarr['nsid'] : $sid;
	}elseif($nsid == -1) $nsid = 0;
	...
	$cnstr = cnstr($midarr);
	$item = cn_parsearr($cnstr,$nsid,$listby);//子站id会在其中转换
...
function farc_sqlstr(&$tag,$type='c',$iscount=0){
	...
	$sqlorder = $sqllimit = '';
	$sqlwhere = " WHERE a.checked=1 AND a.caid=".$tag['casource'];
...
function one_freeurl(&$tag){
	...
	if(empty($freeinfos[$fid]['arcurl'])){
		$freeurl = $cms_abs."freeinfo.php?fid=$fid";
	}else $freeurl = ($sid ? (empty($subsites[$sid]['smallsite']) ? $cms_abs.$subsites[$sid]['dirname'].'/' : $subsites[$sid]['smallsite']) : $cms_abs.$infohtmldir.'/').$freeinfos[$fid]['arcurl'];
	unset($fid,$sid);
	return $freeurl;
}
...
function arr_mcatalogs(&$tag){
	...
	if($tag['listby'] == 'ca'){//全部栏目
		...
		foreach($rets as $k => $v){
			$v['indexurl'] = mcn_url($v['mcaid']);
			$v['listurl'] = mcn_url($v['mcaid'],0,1);
			$v['sn_row'] = $i = empty($i) ? 1 : $i+1;
			$rets[$k] = $v;
		}
		...
		if(!empty($_midarr['mcaid'])){//一定必须是激活栏目下的分类
			foreach($uclasses as $k => $v){
				if($v['mcaid'] == $_midarr['mcaid']){
					$v['indexurl'] = mcn_url($v['mcaid'],$v['ucid']);
					$v['listurl'] = mcn_url($v['mcaid'],$v['ucid'],1);
					$v['sn_row'] = $i = empty($i) ? 1 : $i+1;
					$rets[] = $v;
				}
			}
...
function mpnav($temparr=array(),$s_num=0){
	...
	$mpnav = '';
	$mpstart = m_parseurl($static ? $surlpre : $durlpre,array('page' => 1),0);
	$mpend = m_parseurl($static && $pcount <= $s_num ? $surlpre : $durlpre,array('page' => $pcount),0);
	$mppre = $nowpage > 1 ? m_parseurl($static && ($nowpage - 1) <= $s_num ? $surlpre : $durlpre,array('page' => $nowpage - 1),0) : '#';
	$mpnext = $nowpage < $pcount ? m_parseurl($static && ($nowpage + 1) <= $s_num ? $surlpre : $durlpre,array('page' => $nowpage + 1),0) : '#';
	...
	for($i = $from; $i <= $to; $i++){
		$mpnav .= $i == $nowpage ? '<a class="p_curpage">'.$i.'</a>' :  '<a href="'.m_parseurl($static && $i <= $s_num ? $surlpre : $durlpre,array('page' => $i),0).'" class="p_num">'.$i.'</a>';
	}
...
function arr_nownav(&$tag){
	...
	foreach($idsarr as $k => $v){
		if($k == 'caid'){
			...
			foreach($pids as $id){
				$midarr['ccid'.$coid] = $id;
				if($item = onenav($tag,$midarr,$coid)){
					$item['sn_row'] = $i = empty($i) ? 1 : $i+1;
					$rets[] = $item;
				}
			}
...
function odealtxt($tname,&$tag,&$sarr,$mode='u'){
	if($mode == 'u'){
...
function arr_vote(&$tag){//投票选项列表
	...
	while($row = $db->fetch_array($query)){
		$row['input'] = !$vote['ismulti'] ? "<input type=\"radio\" value=\"".$row['vopid']."\" name=\"vopids[]\">" : "<input type=\"checkbox\" value=\"".$row['vopid']."\" name=\"vopids[]\">";
...
function outinfos_arr(&$tag,$temparr=array(),$type='c',$page=1){
	...
	$tag['sqlstr'] = sqlstr_replace($tag['sqlstr'],$temparr);
	$page = max(1,intval($page));
	$tag['limits'] = empty($tag['limits']) ? 10 : $tag['limits'];
	$tag['sqlstr'] .= " LIMIT ".(($page - 1) * $tag['limits']).",".$tag['limits'];
	$query = $ndb->query($tag['sqlstr']);
	while($row = $ndb->fetch_array($query)){
		$retarr[] = $row;
	}
	return $retarr;
...
function outinfos_nums(&$tag){
	...
	$sqlstr = stristr($tag['sqlstr'],'FROM');
...
function rel_sqlstr(&$tag,$type='c',$iscount=0){
	...
	foreach($cotypes as $k => $cotype){
		...
		if(!empty($tag['cosource'.$k])){
			...
			if(!empty($ccids)){
				if(empty($cotype['self_reg'])){
					$sqlwhere .= " AND ccid$k ".multi_str($ccids);
				}else{
					$tempstr = self_sqlstr($k,$ccids,'');
					$tempstr && $sqlwhere .= ' AND '.$tempstr;
					unset($tempstr);
				} 
			}
...
function one_user(&$tag){
	...
	if(!$tag['usource'] && $memberid){
		$curuser->sub_data();
		$item = $curuser->info;
	}elseif(!empty($_midarr['mid'])){
		$item = $db->fetch_one("SELECT m.*,s.* FROM {$tblprefix}members m LEFT JOIN {$tblprefix}members_sub s ON s.mid=m.mid WHERE m.mid='$_midarr[mid]'");
	}
